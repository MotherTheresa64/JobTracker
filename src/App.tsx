...
import { Task, SortBy, FilterBy, TaskStats } from './types';
...
function App() {
  ...
  const addTask = (taskData: Omit<Task, 'id' | 'createdAt' | 'completed' | 'hammered'>) => {
    const newTask: Task = {
      ...taskData,
      id: uuidv4(),
      createdAt: new Date(),
  ...
  const hammerTask = (id: string, amount: number = 10) => {
    setTasks(prev => prev.map(task => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Header onAddTask={() => setIsFormOpen(true)} />
